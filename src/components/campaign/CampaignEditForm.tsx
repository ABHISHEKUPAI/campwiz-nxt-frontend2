    ...
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      ...
      >
        ...
        <Autocomplete
          options={["commons"]}
          renderInput={(params) => (
            <TextField {...params} label={t("campaign.wikiProject")} variant="outlined" />
          )}
          sx={{ width: { xs: "100%", sm: "40%" }, mb: 1 }}
          value={campaign.language}
          onChange={(e, value) =>dispatch({ language: value ?? "" })}
          disabled={loading || disabled}
        />
        ...
        <DatePicker
          timezone="UTC"
          onChange={(date) =>
            dispatch({ startDate: date?.toISOString() ?? null })
          }
          value={campaign.startDate ? dayjs(campaign.startDate) : null}
        ...
        <DatePicker
          timezone="UTC"
          onChange={(date) =>
            dispatch({ endDate: date?.toISOString() ?? null })
          }
          value={campaign.endDate ? dayjs(campaign.endDate) : null}
          sx={{ width: { xs: "100%", sm: "27%" }, mb: 1 }}
